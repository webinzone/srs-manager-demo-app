...
class ConditionReportsController extends Controller
{
    ...
    public function update($id)
    {
        ...
        $res = ClientDetail::where('id', '=', $id)->firstOrFail();
        $name = $res->fname.". ".$res->mname.". ".$res->lname;
        $condition_report->room = request('room') ?? 'null';
        $condition_report->items = implode(',', (array) request('items')) ?? 'null';
        $condition_report->clean = "null" ?? 'null';
        $condition_report->undamaged = "null" ?? 'null';
        $condition_report->working = "null" ?? 'null';
        $condition_report->prop_comments = "null" ?? 'null';
        $condition_report->res_comments = implode(',', (array) request('res_comments')) ?? 'null';
        $condition_report->res_name = $name ?? 'null';
        $condition_report->stf_name = request('stf_name') ?? 'null';
        $condition_report->res_date = request('res_date') ?? 'null';
        $condition_report->item_no = implode(',', (array) request('item_no')) ?? 'null';
        $condition_report->owned_by = implode(',', (array) request('owned_by')) ?? 'null';
        $condition_report->res_cond = implode(',', (array) request('res_cond')) ?? 'null';
        $condition_report->res_sign = "" ?? 'null';
        $condition_report->st_sign = "" ?? 'null';
        $condition_report->company_id = "null" ?? 'null';
        $condition_report->location_id = "null" ?? 'null';
        $condition_report->user_id =  Auth::user()->id;
